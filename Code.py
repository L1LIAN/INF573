...
def landmarkList2array(landmarkList):
    ...
    for landmark in landmarkList.landmark:
        res.append([[-landmark.z,landmark.x,-landmark.y]]) # we change the ordre of the vectors so that they are in the intuitive order
    return np.array(res)
...
class plot_animation():
    ...
    def animate(self,y_values):
        ...
        for i,y_values in enumerate(self.y_values): 
            plt.plot(self.x_values, y_values,plot_animation.colors[i], label = self.labels[i])
...
class key_pressed:
    ...
    b = 'b'
    n = 'n'
    s = 's' # shoot, while opening your mouth

    def __init__(self,mode):
        self.prev_k = None
        self.k = None #k is key pressed
        self.mode = mode
    ...
    def update(self, phi, theta, psi, var, var_ref, first_capture):
        ...
        if self.k == None:
            if phi>10:
                self.k = key_pressed.l
            elif phi<-10:
                self.k = key_pressed.r
            elif theta > 10:
                self.k = key_pressed.u
            elif theta < -10:
                self.k = key_pressed.d
            elif psi>10:
                self.k = key_pressed.n
            elif psi<-10:
                self.k = key_pressed.b
            elif (not first_capture and var/var_ref > 1.04) :
                self.k = key_pressed.s
        elif self.k == key_pressed.l:
            if phi<5:
                self.k = None
        elif self.k == key_pressed.r:
            if phi >-5 :
                self.k = None
        elif self.k == key_pressed.u :
            if theta<5:
                self.k = None
        elif self.k == key_pressed.d:
            if theta >-5:
                self.k = None
        elif self.k == key_pressed.b:
            if psi<5:
                self.k = None
        elif self.k == key_pressed.n:
            if psi>-5:
                self.k = None
        elif self.k == key_pressed.s:
            if var/var_ref < 1.02:
                self.k = None
        
    def press(self):
        if (self.k!=self.prev_k or self.mode == 'continued_press') and self.k:
            print(self.k)
            # pyautogui.press(self.k)
        # if self.k in ['up','down']:
        #     if self.k == 'up':
        #         mouse.wheel(0.3)
        #     elif self.k == 'down':
        #         mouse.wheel(-0.3)
        # elif self.k and self.k!=self.prev_k:
        #     keyboard.press_and_release(self.k)
...
# key = key_pressed('continued_press')
    ...
    with mp_face_mesh.FaceMesh(
        ...
        min_tracking_confidence=0.5) as face_mesh:
            ...
            while cap.isOpened():
                ...
                c+=1
                if c == 2:      #traitement tous les n_images
                    c=0
                    if not success:
...
# =============================================================================
                        ...
                        values, v0, v1, v2, m, var = PCA(landmarkList2array(results.multi_face_landmarks[0]))
                        x,y,z = v2,v1,v0
